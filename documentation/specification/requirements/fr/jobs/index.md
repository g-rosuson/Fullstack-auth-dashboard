# Jobs

Functional requirements for creating, reading, updating, deleting, scheduling, running, and streaming jobs.

## Lifecycle

- [lifecycle/create.md](./lifecycle/create.md) — `FR-JOBS-CRT-*`, `FR-JOBS-UNQ-*`
- [lifecycle/read.md](./lifecycle/read.md) — `FR-JOBS-LST-*`, `FR-JOBS-GET-*`
- [lifecycle/update.md](./lifecycle/update.md) — `FR-JOBS-UPD-*`
- [lifecycle/delete.md](./lifecycle/delete.md) — `FR-JOBS-DEL-*`

## Schedule

- [schedule/schedule.md](./schedule/schedule.md) — `FR-JOBS-SCH-*`
- [schedule/schedule-status.md](./schedule/schedule-status.md) — `FR-JOBS-SSC-*`
- [schedule/once.md](./schedule/once.md) — `FR-JOBS-ONCE-*`
- [schedule/reboot.md](./schedule/reboot.md) — `FR-JOBS-REBOOT-*`

## Execution

- [execution/execution.md](./execution/execution.md) — `FR-JOBS-RUN-*`, `FR-JOBS-STR-*`
- [execution/stop.md](./execution/stop.md) — `FR-JOBS-STP-*`

## Tools

- [tools/tools.md](./tools/tools.md) — `FR-JOBS-TLR-*`

## Ownership

- [ownership/ownership.md](./ownership/ownership.md) — `FR-JOBS-OWN-*`
