...
- **FR-JOBS-RUN-001** — The system shall execute a job’s tools when the job is run (on create/update without a schedule, on demand, or when the schedule fires).
...
- **FR-JOBS-STR-001** — The system shall provide a live stream of job activity, including which jobs are running and when targets finish, the job finishes, the job fails, or the job run is cancelled.
- **FR-JOBS-STR-003** — The system shall present live running state and execution updates from that stream without requiring a full page reload.
...
- **FR-JOBS-STR-006** — When a job run is cancelled (FR-JOBS-STP-001), the system shall emit a live stream event for that cancellation to the owner.
