...
describe('DatePicker block: label', () => {
    afterEach(() => {
        vi.clearAllMocks();
    });

    it('[CLIENT-UI-FRM-002] / [FR-UI-FRM-002] shows the field label and links it to the date trigger', () => {
...
describe('DatePicker block: value', () => {
    afterEach(() => {
        vi.clearAllMocks();
    });

    it('[CLIENT-UI-DAT-002] / [FR-UI-DAT-002] shows the default placeholder on the trigger when no date is selected', () => {
        renderDatePicker({ value: undefined });

        expect(getDatePickerTrigger()).toHaveTextContent('Pick a date');
    });

    it('[CLIENT-UI-DAT-002] / [FR-UI-DAT-002] shows a custom placeholder when no date is selected', () => {
    ...
    it('[CLIENT-UI-DAT-002] / [FR-UI-DAT-002] shows the selected date in a readable long form on the trigger', () => {
...
describe('DatePicker block: required and invalid', () => {
    afterEach(() => {
        vi.clearAllMocks();
    });

    it('[CLIENT-UI-DAT-003] / [FR-UI-DAT-003] marks the field required and shows a required indicator when required is true', () => {
    ...
    it('[CLIENT-UI-DAT-003] / [FR-UI-DAT-003] reports invalid while required and empty so forms can block submit', async () => {
        renderDatePicker({ value: undefined, required: true });
    ...
    it('[CLIENT-UI-DAT-003] / [FR-UI-DAT-003] clears the custom validity message when a date is selected or required is off', async () => {
    ...
    it('[CLIENT-UI-FRM-003] / [FR-UI-FRM-003] shows a field error and marks the control invalid', () => {
        renderDatePicker({ value: undefined, error: 'Date is required' });

        expect(screen.getByRole('alert')).toHaveTextContent('Date is required');
        expect(getDatePickerTrigger()).toHaveAttribute('aria-invalid', 'true');
    });
});

describe('DatePicker block: disabled', () => {
    afterEach(() => {
        vi.clearAllMocks();
    });

    it('[CLIENT-UI-ACT-004] / [FR-UI-ACT-004] prevents opening the calendar when disabled is true', () => {
        renderDatePicker({ value: undefined, disabled: true });

        const trigger = getDatePickerTrigger();
...
describe('DatePicker block: choosing a date', () => {
    afterEach(() => {
        vi.clearAllMocks();
    });

    it('[CLIENT-UI-DAT-001] / [FR-UI-DAT-001] calls onChange with the chosen day and closes the calendar', async () => {
    ...
    it('[CLIENT-UI-DAT-001] / [FR-UI-DAT-002] updates the trigger label after selection when used as a controlled field', async () => {
        const onChangeSpy = vi.fn();
    ...
    it('[CLIENT-UI-ACT-003] / [FR-UI-ACT-003] defaults the trigger type to button so it does not submit a parent form', async () => {
        const onSubmit = vi.fn(event => event.preventDefault());

        render(
            <form onSubmit={onSubmit}>
                <DatePicker label={defaultLabel} value={undefined} onChange={vi.fn()} />
            </form>
        );
        ...
        expect(getDatePickerTrigger()).toHaveAttribute('type', 'button');
    });
});
