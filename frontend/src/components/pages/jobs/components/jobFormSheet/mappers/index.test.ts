...
import mappers from '.';
import { JobScheduleStatus, JobScheduleType, type Tool } from '@/_types/_gen';
...
const buildState = (overrides: Partial<JobFormSheetState> = {}): JobFormSheetState => ({
    name: 'Test job',
    scheduleType: '',
    scheduleStatus: JobScheduleStatus.idle,
...
describe('JobFormSheet mappers: mapToUpdateJobPayload', () => {
    ...
    it('includes schedule status on the update payload from form state', () => {
        const state = buildState({
            scheduleType: JobScheduleType.daily,
            scheduleStatus: JobScheduleStatus.stopped,
            tools: [],
        });

        expect(mappers.mapToUpdateJobPayload(state).schedule?.status).toBe(JobScheduleStatus.stopped);
    });
